...
accreal THTensor_(prodall)(THTensor *tensor)
{
  accreal prod = 1;
#ifdef _OPENMP
  TH_TENSOR_APPLY_REDUCTION_ADVANCED_INDEX(real, tensor, *:prod, prod *= *tensor_data;);
#else
...
#ifdef _OPENMP
#if defined(TH_REAL_IS_BYTE)
      TH_TENSOR_APPLY2_ADVANCED_INDEX2(r_Size, r_Contig, tContig, real, r_, real, t, *r__data = (((real) *t_data) << value);, rp[iter] = (((real) tp[iter]) << value););
#else
      TH_TENSOR_APPLY2_ADVANCED_INDEX2(r_Size, r_Contig, tContig, real, r_, real, t, *r__data = (((unsigned real) *t_data) << value);, rp[iter] = (((unsigned real) tp[iter]) << value););
#endif
#else
#if defined(TH_REAL_IS_BYTE)
...
#ifdef _OPENMP
#if defined(TH_REAL_IS_FLOAT) || defined(TH_REAL_IS_DOUBLE)
      TH_TENSOR_APPLY2_ADVANCED_INDEX2(r_Size, r_Contig, tContig, real, r_, real, t, *r__data = fmod(*t_data, value);, rp[iter] = fmod(tp[iter], value););
#else
      TH_TENSOR_APPLY2_ADVANCED_INDEX2(r_Size, r_Contig, tContig, real, r_, real, t, *r__data = (*t_data % value);, rp[iter] = (tp[iter] % value););
#endif
#else
  ...
  if (tSize == r_Size){
    if (r_Contig && tContig) {
...
#ifdef _OPENMP
#if defined(TH_REAL_IS_FLOAT) || defined(TH_REAL_IS_DOUBLE)
      TH_TENSOR_APPLY2_ADVANCED_INDEX2(r_Size, r_Contig, tContig, real, r_, real, t, *r__data = (value == 0)? NAN : *t_data - value * floor(*t_data / value);, rp[iter] = (value == 0)? NAN : tp[iter] - value * floor(tp[iter] / value););
#else
       // There is no NAN for integers
      TH_TENSOR_APPLY2_ADVANCED_INDEX2(r_Size, r_Contig, tContig, real, r_, real, t, *r__data = *t_data % value;
                                          ...
                                          rp[iter] = tp[iter] % value;
                                          if (rp[iter] * value < 0) rp[iter] += value;);
#endif
#else
#if defined(TH_REAL_IS_FLOAT) || defined(TH_REAL_IS_DOUBLE)
  ...
  if (tSize == r_Size){
    if (r_Contig && tContig) {
      ...
      TH_TENSOR_APPLY2_ADVANCED_INDEX2(r_Size, r_Contig, tContig, real, r_, real, t, *r__data = (*t_data < min_value) ? min_value : (*t_data > max_value ? max_value : *t_data);, 
                                                                                     rp[iter] = (tp[iter] < min_value) ? min_value : (tp[iter] > max_value ? max_value : tp[iter]););
#else
...
#if _OPENMP
        TH_TENSOR_APPLY3_ADVANCED_INDEX2(r_Size, r_Contig, tContig, srcContig, real, r_, real, t, real, src, *r__data = *t_data + value * *src_data;, rp[iter] = tp[iter] + value * srcp[iter];);
#else
...
#if _OPENMP
      TH_TENSOR_APPLY3_ADVANCED_INDEX2(r_Size, r_Contig, tContig, srcContig, real, r_, real, t, real, src, *r__data = *t_data * *src_data;, rp[iter] = tp[iter] * srcp[iter];);
#else
...
#if _OPENMP
      TH_TENSOR_APPLY3_ADVANCED_INDEX2(r_Size, r_Contig, tContig, srcContig, real, r_, real, t, real, src, *r__data = pow(*t_data, *src_data);, rp[iter] = pow(tp[iter], srcp[iter]););   
#else
...
#if defined(TH_REAL_IS_FLOAT)
      TH_TENSOR_APPLY3_ADVANCED_INDEX2(r_Size, r_Contig, tContig, srcContig, real, r_, real, t, real, src, *r__data = *t_data * powf(2, *src_data);, rp[iter] = tp[iter] * powf(2, srcp[iter]););
#elif defined(TH_REAL_IS_DOUBLE)
      TH_TENSOR_APPLY3_ADVANCED_INDEX2(r_Size, r_Contig, tContig, srcContig, real, r_, real, t, real, src, *r__data = *t_data * pow(2, *src_data);, rp[iter] = tp[iter] * pow(2, srcp[iter]););
#elif defined(TH_REAL_IS_BYTE)
      TH_TENSOR_APPLY3_ADVANCED_INDEX2(r_Size, r_Contig, tContig, srcContig, real, r_, real, t, real, src, *r__data = ((real)*t_data) << *src_data;, rp[iter] = ((real)tp[iter]) << srcp[iter];);
#else
      TH_TENSOR_APPLY3_ADVANCED_INDEX2(r_Size, r_Contig, tContig, srcContig, real, r_, real, t, real, src, *r__data = ((unsigned real)*t_data) << *src_data;, rp[iter] = ((unsigned real)tp[iter]) << srcp[iter];);
#endif
...
void THTensor_(crshift)(THTensor *r_, THTensor *t, THTensor *src)
{
  ...
  if (srcSize == r_Size){
    if (r_Contig && tContig && srcContig) {
      ...
      TH_TENSOR_APPLY3_ADVANCED_INDEX2(r_Size, r_Contig, tContig, srcContig, real, r_, real, t, real, src, *r__data = *t_data / powf(2, *src_data);, rp[iter] = tp[iter] / powf(2, srcp[iter]););
#elif defined(TH_REAL_IS_DOUBLE)
      TH_TENSOR_APPLY3_ADVANCED_INDEX2(r_Size, r_Contig, tContig, srcContig, real, r_, real, t, real, src, *r__data = *t_data / pow(2, *src_data);, rp[iter] = tp[iter] / powf(2, srcp[iter]););
#elif defined(TH_REAL_IS_BYTE)
      TH_TENSOR_APPLY3_ADVANCED_INDEX2(r_Size, r_Contig, tContig, srcContig, real, r_, real, t, real, src, *r__data = ((real)*t_data) >> *src_data;, rp[iter] = ((real)tp[iter]) >> srcp[iter];);
#else
      TH_TENSOR_APPLY3_ADVANCED_INDEX2(r_Size, r_Contig, tContig, srcContig, real, r_, real, t, real, src, *r__data = ((unsigned real)*t_data) >> *src_data;, rp[iter] = ((unsigned real)tp[iter]) >> srcp[iter];);
...
#if defined(TH_REAL_IS_FLOAT) || defined(TH_REAL_IS_DOUBLE)
      TH_TENSOR_APPLY3_ADVANCED_INDEX2(r_Size, r_Contig, tContig, srcContig,real, r_, real, t, real, src, *r__data = fmod(*t_data, *src_data);, rp[iter] = fmod(tp[iter], srcp[iter]););
#else
     TH_TENSOR_APPLY3_ADVANCED_INDEX2(r_Size, r_Contig, tContig, srcContig, real, r_, real, t, real, src, *r__data = (*t_data % *src_data);, rp[iter] = (tp[iter] % srcp[iter]););
#endif
  ...
  if (srcSize == r_Size){
    if (r_Contig && tContig && srcContig) {
     ...
     TH_TENSOR_APPLY3_ADVANCED_INDEX2(r_Size, r_Contig, tContig, srcContig, real, r_, real, t, real, src, *r__data = (*src_data == 0)? NAN : *t_data - *src_data * floor(*t_data / *src_data);,
                                                                                                          rp[iter] = (srcp[iter] == 0)? NAN : tp[iter] - srcp[iter] * floor(tp[iter] / srcp[iter]););
#else
     TH_TENSOR_APPLY3_ADVANCED_INDEX2(r_Size, r_Contig, tContig, srcContig, real, r_, real, t, real, src, *r__data = *t_data % *src_data;
                                                     ...
                                                     rp[iter] = tp[iter] % srcp[iter];
                                                     if (rp[iter] * srcp[iter] < 0) rp[iter] += srcp[iter];);
#endif
...
#ifdef _OPENMP
      TH_TENSOR_APPLY2_ADVANCED_INDEX2(r_Size, r_Contig, tContig, real, r_, real, t, *r__data = pow(value, *t_data);, rp[iter] = pow(value, tp[iter]););
#else
...
#ifdef _OPENMP
    TH_TENSOR_APPLY3_ADVANCED_INDEX2(r_Size, r_Contig, src1Contig, src2Contig, real, r_, real, src1, real, src2, *r__data += value * *src1_data * *src2_data;, rp[iter] += value * tp[iter] * srcp[iter];);
#else
...
#define LAB_IMPLEMENT_BASIC_CODE(NAME, CODE, VCODE)             \
  void THTensor_(NAME)(THTensor *r_, THTensor *t)             \
  {                                                           \
    THTensor_(resizeAs)(r_, t);                               \
    ptrdiff_t r_Size = THTensor_(nElement)(r_);                 \
    ...
    int tContig = THTensor_(isContiguous)(t)? 1:0;            \
    if( (tSize == r_Size) && (r_Size > TH_OMP_OVERHEAD_THRESHOLD) ){                                 \
      TH_TENSOR_APPLY2_ADVANCED_INDEX2(r_Size, r_Contig, tContig, real, t, real, r_, CODE, VCODE);                 \
    }                                                                                                 \
    else {                                                                                            \

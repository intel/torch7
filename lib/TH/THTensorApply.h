    ...
    if((TENSOR2##StrideContg != 0) && (TENSOR1##StrideContg != 0) ){          \
      ...
      if(CONTIG1 && CONTIG2){                                    \
        ...
        for (iter=0; iter<(SIZE); iter++) { \
          ...
          ptrdiff_t TENSOR2##BasicIndex = 0;\
          for(dim = 0; dim < TENSOR2##Dim-1; dim++) {\
            quot = rem/TENSOR2##Stride[dim+1];\
            rem = rem%TENSOR2##Stride[dim+1];\
            TENSOR2##BasicIndex += quot*TENSOR2->stride[dim];\
          }\
          TENSOR2##BasicIndex += rem*TENSOR2->stride[dim];\
                                                             \
...
\
          for(dim = 0; dim < TENSOR1##Dim-1; dim++) {\
            quot = rem/TENSOR1##Stride[dim+1];\
            rem = rem%TENSOR1##Stride[dim+1];\
            TENSOR1##BasicIndex += quot*TENSOR1->stride[dim];\
          }\
          TENSOR1##BasicIndex += rem*TENSOR1->stride[dim];\
          TYPE2 *TENSOR2##_data = tp+iter;\
...
#define TH_TENSOR_APPLY3_ADVANCED_INDEX3(SIZE, CONTIG1, CONTIG2, CONTIG3, TYPE1, TENSOR1, TYPE2, TENSOR2, TYPE3, TENSOR3, CODE) \
{                                                                             \
  int TENSOR1##Dim = TENSOR1->nDimension;                                     \
  int TENSOR2##Dim = TENSOR2->nDimension;                                     \
...
\
          for(dim = 0; dim < TENSOR3##Dim-1; dim++) {\
            quot = rem/TENSOR3##Stride[dim+1];\
            rem = rem%TENSOR3##Stride[dim+1];\
            TENSOR3##BasicIndex += quot*TENSOR3->stride[dim];\
          }\
          TENSOR3##BasicIndex += rem*TENSOR3->stride[dim];\
